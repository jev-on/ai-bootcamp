...
## Path B (Manual): Google (Gemini Advanced)

Want to see how the "Magic" works? Here is how to build the loop yourself using Gemini.

Create `weather_agent_gemini_manual.py`:
...
def run_agent(user_message):
    print(f"Thinking about: {user_message}...")
    
    # Step 1: Send message + tools
    # We turn OFF auto-execution so we can see the request
    response = client.models.generate_content(
        model="gemini-3-flash",
        contents=user_message,
        config={"tools": tools, "tool_config": {"function_calling_config": {"mode": "ANY"}}}
    )

    # Step 2: Check for Tool Calls
    # Gemini returns a list of 'parts'. We check if any part is a function call.
    if response.function_calls:
        for call in response.function_calls:
            print(f"--- [AI WANTS TOOL] {call.name} with args {call.args}")
            
            # Step 3: Run the Python code manually
            if call.name == "get_coordinates":
                result = get_coordinates(call.args["city_name"])
            elif call.name == "get_weather":
                result = get_weather(call.args["latitude"], call.args["longitude"])
            
            # Step 4: Send the result back to Gemini
            # We must construct a 'function_response' part
            print(f"--- [SENDING DATA] {result}")
            
            # In the manual loop, we need to send the history:
            # 1. User User Message
            # 2. AI's Function Call
            # 3. Our Function Response
            # For simplicity in this demo, we just print the result. 
            # (Building the full history object manually in Gemini V1 is complex, 
            # which is why everyone uses the Auto-Chat mode!)
